...
def get_all_users_accuracy():
    """Get accuracy for all tracked users: Brian, Chris, Jason, Mike, Patti."""
    tracked_users = ["Brian", "Chris", "Jason", "Mike", "Patti"]
    game_results = st.session_state.get("game_results", {})
    user_accuracy = []
    ...
    for user in tracked_users:
        ...
        for game_id, actual_winner in game_results.items():
            if game_id in user_picks:
                if user_picks[game_id] == actual_winner:
                    correct += 1
                else:
                    incorrect += 1

        accuracy_pct = (correct / (correct + incorrect) * 100) if (correct + incorrect) > 0 else 0.0
...
if st.sidebar.button("📊", key="show_leaderboard", help="View Leaderboard", use_container_width=False):
    st.session_state.show_leaderboard = True

if st.session_state.get("show_leaderboard", False):
    st.sidebar.markdown("---")
    if st.sidebar.button("← Back to Teams", key="hide_leaderboard", use_container_width=True):
    ...
    if leaderboard_data:
        ...
        for idx, user_data in enumerate(leaderboard_data, start=1):
            leaderboard_rows.append({
                ...
                "Accuracy %": f"{user_data['accuracy']:.1f}%"
            })
        import pandas as pd
